...
import soulfind.defines : blue, bold, check_user_interval, kick_duration,
                          ...
                          VERSION;
import soulfind.select : DefaultSelector, SelectEvent;
...
class Server
{
    ...
    this(string db_filename, ushort port = 0)
    {
        ...
        this.selector          = DefaultSelector(1.seconds);
        ...
        while (running) {
            ...
            foreach (sock, events ; ready_socks) {
                ...
                if (sock is listen_sock) {
                    if (recv_ready) accept_connections(listen_sock);
                    continue;
                }
    ...
    private void accept_connections(Socket listen_sock)
    {
        while (true) {
            Socket new_sock;
            try
                new_sock = listen_sock.accept();
            ...
            selector.register(new_sock, SelectEvent.read);
        }
    }
