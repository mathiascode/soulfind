...
void parse_args(string[] args, CLIOption[] options)
{
    size_t i = 1;
    while (i < args.length) {
        string arg = args[i];
        ...
        if (arg.startsWith(l_prefix)) {
            ...
            throw new CLIException("Unexpected positional argument: " ~ arg);
        }
        ...
        if (option.arg_name.length > 0) {
            if (found_equals) {
                ...
                throw new CLIException("Missing value for option: " ~ arg);
            }
        }
        else {
...
void print_help(string description, CLIOption[] options)
{
